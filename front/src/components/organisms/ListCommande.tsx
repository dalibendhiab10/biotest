        ...
        <table className="min-w-full">
            <thead className="bg-green-green">
                <tr className="text-white">
                    ...
                    <th className="px-4 py-2 text-center">Id Patient</th>
            ...
            <tbody>
                {commandes.map((commande) => (
                    <tr key={commande.id} className="bg-white shadow-xl rounded-xl
                    border-t-[8px] border-[#f9f9f9] mb-[15xp]" >
                        {  (
                            <>
                                <td className="px-4 py-2 text-center">{commande.analyse.codeCNAM}</td>
                                <td className="px-4 py-2 flex justify-center">
    <img src={commande.biologiste.logo} width={"90px"} height={"90px"} alt="Logo Laboratoire" /> </td>
                                ...
                                <td className="px-4 py-2 text-center"> {commande.patient.id}  </td>
                              
                                <td className="px-4 py-2 font-bold text-center"> {commande.prix_total}DT </td>
                                <td className="px-4 py-2">
                                <div className="flex items-center space-x-4 justify-center">
                                    <button onClick={()=>{onOpenInfo();setCommandeId(commande.id)}} className="flex items-center justify-around font-medium text-5xl flex w-7 h-7 rounded-full 
